...
export interface KatakanaItemProgress {
  quizAttempts: number;
  quizCorrect: number;
...
interface KatakanaProgressState {
  version: 1;
  items: Record<string, KatakanaItemProgress>;
  activeDates: string[];
}

const STORAGE_KEY = 'viejap.katakana-progress.v1';

function emptyState(): KatakanaProgressState {
  return { version: 1, items: {}, activeDates: [] };
}

function emptyItemProgress(): KatakanaItemProgress {
  return {
    quizAttempts: 0,
    quizCorrect: 0,
    quizIncorrect: 0,
...
export class KatakanaProgressService {
  private readonly progressState = signal<KatakanaProgressState>(this.load());
  ...
  getItem(itemId: string): KatakanaItemProgress {
    return this.progressState().items[itemId] ?? emptyItemProgress();
  }

  private updateItem(
    itemId: string,
    updater: (current: KatakanaItemProgress) => KatakanaItemProgress,
  ): void {
  ...
  private load(): KatakanaProgressState {
    ...
    try {
      ...
      const parsed = JSON.parse(raw) as Partial<KatakanaProgressState>;
      if (parsed.version !== 1 || !parsed.items || !Array.isArray(parsed.activeDates)) {
        return emptyState();
      }
      return {
        version: 1,
        items: parsed.items as Record<string, KatakanaItemProgress>,
...
export const KATAKANA_PROGRESS_STORAGE_KEY = STORAGE_KEY;
